...
export function initialiseAttestationConnectorFactory(
	...
	const type = options.envVars.GTSC_ATTESTATION_CONNECTOR;
	...
	if (type === "iota") {
		connector = new IotaAttestationConnector({
			identityConnectorType: options.envVars.GTSC_IDENTITY_CONNECTOR,
			nftConnectorType: options.envVars.GTSC_NFT_CONNECTOR
		});
